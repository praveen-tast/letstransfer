...
EOM;
	...
	public function generateActiveRow($modelClass, $column)
	{
		if ($column->isForeignKey) {
			...
			return 
			"\$form->radioButtonListRow(\$model, '{$column->name}', GxHtml::listDataEx({$relatedModelClass}::model()->findAllAttributes(null, true)))";
		}
		...
		if (strtoupper($column->dbType) == 'TINYINT(1)'
			...
			return "\$form->timepickerRow(\$model, '{$column->name}',
			 array('hint'=>'Click inside!to select time ',
			 'append'=>'<i class=\"icon-time\" style=\"cursor:pointer\"></i>'))\n";
			
		} elseif (preg_match('/^status|status_id|state|state_id$/i', $column->name))
		{
			return "\$form->dropDownListRow(\$model, '{$column->name}',
		...
		elseif (preg_match('/_list$/i', $column->name))
		{
			return "\$form->select2Row(\$model, '{$column->name}',
			...
			return " '';\$code = \$this->richTextEditor() ;\n
					if (\$code == 1) echo \$form->html5EditorRow(\$model,'{$column->name}', array('class'=>'span4', 'rows'=>5, 'height'=>'200', 'options'=>array('color'=>true)));\n
					else if (\$code == 2) echo \$form->redactorRow(\$model,'{$column->name}', array('class'=>'span4', 'rows'=>5));\n
					else if (\$code == 3) echo \$form->ckEditorRow(\$model,'{$column->name}', array('options'=>array('fullpage'=>'js:true', 'width'=>'640', 'resize_maxWidth'=>'640','resize_minWidth'=>'320')));\n
					else echo \$form->textAreaRow(\$model,'{$column->name}',  array('class'=>'span4', 'rows'=>5));";
